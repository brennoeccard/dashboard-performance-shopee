...
DUAL_THEME = dict(plot_bgcolor="#0f0d0b", paper_bgcolor="#0f0d0b", font_color="#f6e8d8",
...
def calcular(df):
    ...
    lucro_total=comissao; roi=0; ctr_shopee=(vendas/cliques*100) if cliques>0 else 0
    ticket_medio=comissao/vendas if vendas>0 else 0
    return dict(cliques=cliques,vendas=vendas,comissao=comissao,invest=0,
                impressoes=0,alcance=0,cliques_meta=0,lucro_total=lucro_total,roi=roi,
                ctr_shopee=ctr_shopee,ctr_meta=0,ctr_cv=0,freq=0,
                cpm_imp=0,cpm_alc=0,cpc=0,cac=0,ticket_medio=ticket_medio)
...
def ler_pago():
    ...
    vals=res.get("values",[])
    if len(vals)<2: return pd.DataFrame()
    cab=vals[0]; linhas=vals[1:]; mc=max(len(l) for l in [cab]+linhas)
    ln=[l+[""]*(mc-len(l)) for l in linhas]
...
def check_login():
    ...
    if not st.session_state.logged_in:
        ...
        with c2:
            ...
            <p style='color:#bd6d34;'>por Carol Matos</p></div>""",unsafe_allow_html=True)
...
def dual_chart(df, x_col, bar_col, line_col, title, y1_label, y2_label, bar_color="#c0392b", line_color="#bd6d34"):
    ...
    fig.update_layout(title=title,hovermode="x unified",
        ...
        yaxis2=dict(title=y2_label,overlaying="y",side="right",color=line_color),
        **DUAL_THEME)
    return fig
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("""
        <a href="#kpis-gerais" style="display:block;color:#c5936d;font-size:12px;text-decoration:none;background:#1a1210;padding:6px 12px;border-radius:8px;border:1px solid #3a2c28;margin-bottom:4px;text-align:center;">💰 KPIs Gerais</a>
        <a href="#campanha-pago" style="display:block;color:#c5936d;font-size:12px;text-decoration:none;background:#1a1210;padding:6px 12px;border-radius:8px;border:1px solid #3a2c28;margin-bottom:4px;text-align:center;">📣 Campanha Pago</a>
    ...
    st.markdown('<h1 style="color:#f6e8d8;margin:0 0 4px 0;font-size:28px;">📊 Dashboard de Performance</h1><p style="color:#c5936d;margin:0 0 16px 0;font-size:13px;">Destrava · por Carol Matos</p>',unsafe_allow_html=True)
    ...
    with st.expander("🎛️ Filtros",expanded=False):
        ...
        st.markdown('<div style="color:#bd6d34;font-size:12px;font-weight:700;margin-bottom:4px;">Canal</div>',unsafe_allow_html=True)
    ...
    # FILTRAR PAGO — por data, sem filtro de canal (pago é sempre pago)
    if not df_pago_raw.empty:
        _d_ini=pd.Timestamp(d_ini).date(); _d_fim=pd.Timestamp(d_fim).date()
        mask_p=(df_pago_raw["Data"].dt.date>=_d_ini)&(df_pago_raw["Data"].dt.date<=_d_fim)
        if sid1_sel!=sid1_opts: mask_p=mask_p&df_pago_raw["Sub_id1"].isin(sid1_sel)
        df_pago_periodo=df_pago_raw[mask_p].copy()
    else:
    ...
    if df.empty:
        st.markdown('<div style="background:#1a1210;border:1px solid #3a2c28;border-radius:10px;padding:24px;text-align:center;"><div style="font-size:32px;">📭</div><div style="color:#f6e8d8;">Sem dados para o periodo.</div></div>',unsafe_allow_html=True)
        st.stop()

    # CALCULAR INVESTIMENTOS (fontes limpas, sem merge)
    ...
    # METRICAS DE CAMPANHA DO DF_PAGO_PERIODO
    imp_pago=df_pago_periodo["Impressoes"].sum() if not df_pago_periodo.empty else 0.0
    alc_pago=df_pago_periodo["Alcance"].sum() if not df_pago_periodo.empty else 0.0
    clq_meta=df_pago_periodo["Cliques_Meta"].sum() if not df_pago_periodo.empty else 0.0
    ...
    m["impressoes"]=imp_pago; m["alcance"]=alc_pago; m["cliques_meta"]=clq_meta
    m["ctr_meta"]=(clq_meta/alc_pago*100) if alc_pago>0 else 0
    m["ctr_cv"]=(m["vendas"]/clq_meta*100) if clq_meta>0 else 0
    m["freq"]=imp_pago/alc_pago if alc_pago>0 else 0
    m["cpm_imp"]=(invest_pago/imp_pago*1000) if imp_pago>0 else 0
    m["cpm_alc"]=(invest_pago/alc_pago*1000) if alc_pago>0 else 0
    m["cpc"]=invest_pago/clq_meta if clq_meta>0 else 0
    ...
    if m_pago:
        m_pago["invest"]=invest_pago; m_pago["lucro_total"]=m_pago["comissao"]-invest_pago
        m_pago["roi"]=(m_pago["comissao"]-invest_pago)/invest_pago if invest_pago>0 else 0
        m_pago["impressoes"]=imp_pago; m_pago["alcance"]=alc_pago; m_pago["cliques_meta"]=clq_meta
        m_pago["cpm_imp"]=(invest_pago/imp_pago*1000) if imp_pago>0 else 0
        m_pago["cpm_alc"]=(invest_pago/alc_pago*1000) if alc_pago>0 else 0
        m_pago["cpc"]=invest_pago/clq_meta if clq_meta>0 else 0
        m_pago["cac"]=invest_pago/m_pago["vendas"] if m_pago["vendas"]>0 else 0
        m_pago["ctr_meta"]=(clq_meta/alc_pago*100) if alc_pago>0 else 0
        m_pago["ctr_cv"]=(m_pago["vendas"]/clq_meta*100) if clq_meta>0 else 0
        m_pago["freq"]=imp_pago/alc_pago if alc_pago>0 else 0
    ...
    # KPIs GERAIS
    # ════════════════════════════════════════
    st.markdown('<div id="kpis-gerais" class="section-title">💰 KPIs Gerais</div>',unsafe_allow_html=True)
    ...
    # ════════════════════════════════════════
    # CAMPANHA PAGO — KPIs + Graficos
    # ════════════════════════════════════════
    if m_pago:
        st.markdown('<div id="campanha-pago" class="section-title">📣 Campanha Pago</div>',unsafe_allow_html=True)
        ...
        with k7:  card("CPC",fmt_brl(m_pago.get("cpc",0)),"blue",delta_html(m_pago.get("cpc",0),mp.get("cpc",0)))
    ...
    if not df_aw.empty:
        ...
        imp_aw_ant=df_aw_ant["Impressoes_aw"].sum() if not df_aw_ant.empty else 0
        ...
        cpm_aw_ant=(inv_aw_ant/imp_aw_ant*1000) if imp_aw_ant>0 else 0
        cpa_aw_ant=(inv_aw_ant/vis_aw_ant) if vis_aw_ant>0 else 0
        cps_aw_ant=(inv_aw_ant/seg_aw_ant) if seg_aw_ant>0 else 0
        cpc_aw_ant=(inv_aw_ant/com_aw_ant) if com_aw_ant>0 else 0
        aw1,aw2,aw3,aw4=st.columns(4)
        with aw1: card("Invest. Awareness",fmt_brl(inv_aw),"red",delta_html(inv_aw,inv_aw_ant))
        with aw2: card("Impressoes",fmt_num(int(imp_aw)),"yellow",delta_html(imp_aw,imp_aw_ant))
        ...
        aw5,aw6,aw7,aw8,aw9=st.columns(5)
        with aw5: card("Comentarios",fmt_num(int(com_aw)),"blue",delta_html(com_aw,com_aw_ant))
        with aw6: card("CPM",fmt_brl(cpm_aw),"yellow",delta_html(cpm_aw,cpm_aw_ant))
        with aw7: card("Custo/Visita",fmt_brl(cpa_aw),"orange",delta_html(cpa_aw,cpa_aw_ant))
        with aw8: card("Custo/Seguidor",fmt_brl(cps_aw),"purple",delta_html(cps_aw,cps_aw_ant))
        with aw9: card("Custo/Comentario",fmt_brl(cpc_aw),"blue",delta_html(cpc_aw,cpc_aw_ant))
        # Grafico awareness
        ...
        metricas_aw={"Visitas ao Perfil":("Visitas","CPA","Visitas","Custo/Visita (R$)","#bd6d34"),
                     "Impressoes":("Impressoes","CPM","Impressoes","CPM (R$)","#c5936d"),
                     "Seguidores":("Seguidores","CPS","Seguidores","Custo/Seguidor (R$)","#9c5834"),
                     "Comentarios":("Comentarios","CPC_aw","Comentarios","Custo/Comentario (R$)","#d2b095")}
        met_aw=st.selectbox("Metrica Awareness",list(metricas_aw.keys()),key="sel_aw")
        col_vol,col_custo,y2v,y2c,cor_aw=metricas_aw[met_aw]
        st.plotly_chart(dual_chart(df_aw_d,"Data","Invest",col_vol,"Investimento vs {}".format(met_aw),"Investimento (R$)",y2v,line_color=cor_aw),use_container_width=True)
        ...
        df_aw_s=df_aw_d[["Data","Invest"]].copy(); df_aw_s["Data"]=df_aw_s["Data"]+pd.Timedelta(days=3)
        ...
        if len(df_imp)>3 and df_imp["Invest_lag"].sum()>0:
            ...
            st.markdown('<div style="background:#1a1210;border:1px solid #3a2c28;border-radius:8px;padding:10px 14px;margin-bottom:8px;"><span style="color:#c5936d;font-size:11px;">Correlacao Awareness -> Vendas Org/Story (lag 3d): </span><span style="color:{};font-size:14px;font-weight:700;">{:.2f}</span> <span style="color:#c5936d;font-size:11px;">— {}</span></div>'.format(cor_txt,corr,interp),unsafe_allow_html=True)
            if len(df_imp)>=7:
                df_imp["corr_movel"]=df_imp["Invest_lag"].rolling(7,min_periods=3).corr(df_imp["Vendas"])
                df_imp_c=df_imp.dropna(subset=["corr_movel"])
                if not df_imp_c.empty:
                    _aw_t=dict(plot_bgcolor="#0f0d0b",paper_bgcolor="#0f0d0b",font_color="#f6e8d8",legend=dict(font=dict(color="#f6e8d8",size=12),bgcolor="rgba(30,18,16,0.8)"))
                    fig_ct=go.Figure()
                    fig_ct.add_trace(go.Scatter(x=df_imp_c["Data"],y=df_imp_c["corr_movel"],mode="lines+markers",line=dict(color="#bd6d34",width=2),fill="tozeroy",fillcolor="rgba(189,109,52,0.1)",name="Correlacao MM7"))
                    ...
                    fig_ct.update_layout(title="Tendencia de Correlacao Awareness -> Vendas (janela 7d)",yaxis=dict(title="Correlacao",color="#c5936d",gridcolor="#2a1f1a"),**_aw_t)
                    st.plotly_chart(fig_ct,use_container_width=True)
            if len(df_imp)>=7:
                df_imp["corr_m"]=df_imp["Invest_lag"].rolling(7,min_periods=3).corr(df_imp["Vendas"])
                df_ic=df_imp.dropna(subset=["corr_m"])
                if not df_ic.empty:
                    fig_ct=go.Figure()
                    fig_ct.add_trace(go.Scatter(x=df_ic["Data"],y=df_ic["corr_m"],mode="lines+markers",line=dict(color="#bd6d34",width=2),fill="tozeroy",fillcolor="rgba(189,109,52,0.1)",name="Correlacao MM7"))
                    fig_ct.add_hline(y=0.3,line_dash="dash",line_color="#7a9e4e",annotation_text="Positivo (0.3)")
                    fig_ct.add_hline(y=0,line_dash="dot",line_color="#c5936d")
                    fig_ct.update_layout(title="Tendencia Correlacao Awareness -> Vendas",yaxis=dict(title="Correlacao",color="#c5936d",gridcolor="#2a1f1a"),**PLOTLY_THEME)
                    st.plotly_chart(fig_ct,use_container_width=True)
    else:
    ...
    if not df_pago_periodo.empty and df_pago_periodo["Impressoes"].sum()>0:
        ...
        with col_funil:
            fl=["Vendas","Cliques Meta","Alcance","Impressoes"]; fv=[vnd_t,clq_t,alc_t,imp_t]
            fp=[vnd_t/imp_t*100 if imp_t>0 else 0,clq_t/imp_t*100 if imp_t>0 else 0,alc_t/imp_t*100 if imp_t>0 else 0,100]
            ...
            for l,v,p in zip(fl,fv,fp):
                fig_f.add_trace(go.Bar(x=[p],y=[l],orientation="h",marker_color=cores_h[l],text="{:,.0f}  ({:.2f}%)".format(v,p),textposition="inside",name=l,showlegend=False))
            fig_f.update_layout(title="Funil",barmode="overlay",**PLOTLY_THEME,height=240,margin=dict(l=0,r=0,t=40,b=0))
            st.plotly_chart(fig_f,use_container_width=True)
            st.markdown('<div style="display:flex;gap:12px;flex-wrap:wrap;">'+"".join(['<span style="font-size:10px;color:#c5936d;"><span style="color:{};font-size:14px;">■</span> {}</span>'.format(cores_b[k],k) for k in cores_b])+'</div>',unsafe_allow_html=True)
        with col_steps:
    ...
    if not df_aw.empty:
        df_aw_x=df_aw.groupby("Data").agg(Invest_aw=("Investimento_aw","sum"),Visitas=("Visitas_Perfil","sum"),Seguidores=("Seguidores","sum"),Comentarios=("Comentarios","sum")).reset_index()
        df_cross=df_cross.merge(df_aw_x,on="Data",how="left").fillna(0)
    else:
    ...
    metricas_disponiveis={
        ...
        "Vendas":"Vendas","Comissao":"Comissao","Cliques":"Cliques",
        "Ticket Medio":"Ticket_Medio","Visitas Perfil":"Visitas",
        "Seguidores":"Seguidores","Comentarios":"Comentarios",
    }
    disp={k:v for k,v in metricas_disponiveis.items() if v in df_cross.columns}
    cx1,cx2=st.columns(2)
    with cx1: met1=st.selectbox("Metrica 1 (barras)",list(disp.keys()),index=0,key="cross1")
    with cx2: met2=st.selectbox("Metrica 2 (linha)",list(disp.keys()),index=3,key="cross2")
        ...
        with st.spinner("A analisar..."):
            try:
                ...
                dados="Periodo:{} a {}\nInvest:{:.2f}|Vendas:{:.0f}|Comissao:{:.2f}|Lucro:{:.2f}|ROI:{:.2f}\nCPM:{:.2f}|CPC:{:.2f}|CAC:{:.2f}|Freq:{:.2f}x\nCTR_Meta:{:.2f}%|CTR_Conv:{:.2f}%\nFunil:{:.0f}imp->{:.0f}alc->{:.0f}clq->{:.0f}vnd".format(
                    ...
                    (m_pago["comissao"] if m_pago else 0)-invest_pago,(m_pago["comissao"] if m_pago else 0)/invest_pago-1 if invest_pago>0 else 0,
                    ...
                    imp_pago,alc_pago,clq_meta,m_pago["vendas"] if m_pago else 0)
